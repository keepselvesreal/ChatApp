...
class ChatSystemState:
    ...
    @staticmethod
    def update(transform_fn: Callable[[Dict[str, Any]], Dict[str, Any]]) -> Dict[str, Any]:
        """변환 함수를 적용하여 상태 업데이트"""
        with ChatSystemState._lock:
            current_state = ChatSystemState._state.copy()
            new_state = transform_fn(current_state)
            ChatSystemState._state = new_state
            return new_state 